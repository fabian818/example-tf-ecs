...
module "ecs_autoscaling" {
  ...
  target_memory = each.value.ecs.target_memory
  target_cpu    = each.value.ecs.target_cpu
  min_capacity  = each.value.ecs.min_scale
  max_capacity  = each.value.ecs.max_scale
}

module "ecs_autoscaling_not_lb" {
  source = "../modules/ecs_autoscaling"
  ...
  cluster_name  = module.ecs_cluster.name
  service_name  = module.ecs_services_not_lb[each.key].name
  target_memory = each.value.ecs.target_memory
  target_cpu    = each.value.ecs.target_cpu
  min_capacity  = each.value.ecs.min_scale
  max_capacity  = each.value.ecs.max_scale
